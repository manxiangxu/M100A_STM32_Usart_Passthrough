...
#error "Please define a frequency band in the compiler options."
        ...
        else
        {
            ...
            {
//                 AdrAckCounter = 0; fix datarate can't dec.
#if defined( USE_BAND_433 ) || defined( USE_BAND_780 ) || defined( USE_BAND_868 )
                if( datarate > LORAMAC_MIN_DATARATE )
                {
                    datarate--;
...
// 		DBG("[%dHz, BW0SF%d, Power:%ddBm]", channel.Frequency, datarate, txPower);
// 		print_message("Send Frame:", LoRaMacBuffer, LoRaMacBufferPktLen, 'X');
